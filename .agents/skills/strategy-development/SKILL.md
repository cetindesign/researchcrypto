...
## Avci profile (breakout/momentum — the Hunter role)
The brief description above ("price breaks a recent range or momentum/volume expands") is the *legacy* framing. For real Avci work, **delegate the signal to the typed pure core** in **avci-breakout-confluence** — `evaluateAvciBreakout(candles1h, candles4h, universeRanks, cfg) -> BreakoutSignal` — and the anti-fakeout confirmation to **avci-volume-volatility-confirmation** (`confirmBreakout`). Don't re-inline breakout math here.

- **The discipline (non-negotiable).** A **34% win rate is NORMAL and often optimal** for a breakout system — it profits from **positive skew** (a few big winners pay for many small losers), not from win rate. The confluence stack (Donchian close-beyond MANDATORY; ADX/ROC/RVOL hard filters; MTF/RS/RSI/MACD boosters behind flags, default OFF) exists to **cut trade COUNT** — enforce `coin_count 2` selectivity and skip chop — **NOT to raise the hit rate**. Stacking filters to "beat the L1 baseline win rate," combined with the tight `trailing_callback_pct 0.8` exit, **truncates the right tail** and can keep expectancy negative even as win rate rises. The real levers are **payoff ratio (avg_win/avg_loss)** and **EXIT slippage on thin Tier-B coins** — see atr-adaptive-exits, avci-taker-entry-slippage-guard, avci-signal-validation-rollout.
- **Signal delegation.** The mandatory trigger is a **closed-bar CLOSE beyond the prior 20-bar Donchian high + ATR buffer** (never a wick, never the forming bar), with ADX(14)≥25 (30 for HIGH) + `+DI>-DI`, ROC(12)≥`min_momentum_pct`, and RVOL≥`min_hacim_carpan` as HARD filters, all ANDed. v1 ships ONLY those four; every booster earns its keep out-of-sample first.
- **Decreasing profit pyramid — refined add rule.** Keep the `multiplier ≤ 0.7`, `max 2–3`, Avci-only, `loss_layer_enabled 0` rule, and now gate each add more tightly: **add only while ROC stays positive AND volatility is NOT expanding further** (`profit_add_step_pct` advance in profit is necessary but not sufficient). Adding into a vol spike concentrates size exactly where momentum crashes cluster — the opposite of what pyramiding-into-strength should do. Size adds per avci-volatility-position-sizing.
- **Unit caveat (still UNVERIFIED).** Whether `profit_add_step_pct` / `layer_trigger_type` is price-% or margin-% remains unconfirmed in the config — read it from the engine code before trusting any pyramid threshold. The same caveat applies to the ~18 new gated Avci keys (ratios vs percents).
- **Does not change the other roles.** Kayikci (cross-sectional trend/breadth, NO layering), Bulucu (regime compass, universe screen), and Safra (market-neutral funding) are unaffected — this profile is Avci-only.
